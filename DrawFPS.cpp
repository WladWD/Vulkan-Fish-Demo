...
Engine::DrawFPS::DrawFPS(const VulkanEngineApplication::VulkanData * vulkanData, const VulkanEngineApplication::VulkanEngineData * vulkanEngineData): vulkanData(vulkanData), vulkanEngineData(vulkanEngineData), drawText(nullptr) {
...
void Engine::DrawFPS::draw(VkCommandBuffer commandBuffer) {
	...
	float scale = vulkanData->mSwapChainImageExtent.width > vulkanData->mSwapChainImageExtent.height ? 0.25 : 0.5;
